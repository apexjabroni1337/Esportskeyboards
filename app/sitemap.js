import { keyboards, proPlayers, allPlayers } from "@/data";

const BASE_URL = "https://esportskeyboards.com";
const slug = (n) => n.toLowerCase().replace(/\+/g, "-plus").replace(/[^a-z0-9]+/g, "-").replace(/-+$/, "");

export default function sitemap() {
  const staticPages = [
    { url: BASE_URL, lastModified: new Date(), changeFrequency: "weekly", priority: 1.0 },
    { url: `${BASE_URL}/keyboards`, lastModified: new Date(), changeFrequency: "weekly", priority: 0.9 },
  ...
  const keyboardPages = keyboards.map((m) => ({
    url: `${BASE_URL}/keyboards/${slug(m.name)}`,
  ...
  // Individual switch pages
  const switches = [...new Set(keyboards.map((m) => m.switchType))];
  const switchPages = switches.map((s) => ({
  ...
  // Keyboard comparison pages (top 15 keyboards, all pairs)
  const topKeyboardsForCompare = [...keyboards].sort((a, b) => b.proUsage - a.proUsage).slice(0, 15);
  const comparePages = [];
  for (let i = 0; i < topKeyboardsForCompare.length; i++) {
    for (let j = i + 1; j < topKeyboardsForCompare.length; j++) {
      comparePages.push({
        url: `${BASE_URL}/compare/${slug(topKeyboardsForCompare[i].name)}-vs-${slug(topKeyboardsForCompare[j].name)}`,
        lastModified: new Date(),
        changeFrequency: "monthly",
        priority: 0.6,
      });
  ...
  // "Best keyboard for X" guide pages
  ...
  const blogSlugs = ["how-to-choose-gaming-keyboard", "hall-effect-vs-mechanical-switches", "keyboard-switch-trend-2024-2026", "wireless-vs-wired-2026", "rapid-trigger-actuation-explained"];
  const blogPages = [
  ...
  return [...staticPages, ...keyboardPages, ...playerPages, ...gamePages, ...brandPages, ...teamPages, ...switchPages, ...shapePages, ...sensitivityPages, ...comparePages, ...bestPages, ...blogPages];
}
