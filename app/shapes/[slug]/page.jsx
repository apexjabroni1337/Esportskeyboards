...
const SHAPES = {
  ...
  ergonomic: {
    ...
    prosAdvice: "Ergonomic keyboards are favored by players who play long sessions or who prioritize comfort over raw speed. Many Counter-Strike veterans who grew up on the Zowie EC or Microsoft IntelliMouse Explorer 3.0 continue to prefer ergonomic layouts even as the meta shifts toward symmetrical designs.",
  },
};
...
export function generateMetadata({ params }) {
  ...
  const shapeMice = keyboards.filter(m => m.layout === shape.title);
  const description = `${shape.title} (${shape.aka}) gaming keyboards — ${shapeMice.length} keyboards reviewed. Pro usage data, top picks, weight and price comparisons. Find the best ${shape.title.toLowerCase()} keyboard for your grip style.`;

  return {
    title: `Best ${shape.title} Gaming Keyboards — ${shapeMice.length} Keyboards Compared`,
    description,
    ...
    openGraph: {
      ...
      images: [{ url: `https://esportskeyboards.com/og?title=Best+${shape.title}+Keyboards&subtitle=${shapeMice.length}+Keyboards+Compared+%C2%B7+Pro+Usage+Data`, width: 1200, height: 630 }],
    },
...
export default function ShapePage({ params }) {
  ...
  return (
    <>
      ...
      >
        <h1>Best {shape.title} ({shape.aka}) Gaming Keyboards for Esports — {shapeMice.length} Keyboards Compared</h1>
        ...
        <table>
          ...
          <tbody>
            <tr><th>Total {shape.title} Keyboards</th><td>{shapeMice.length}</td></tr>
        ...
        <table>
          <caption>{shapeMice.length} {shape.title.toLowerCase()} gaming keyboards ranked by professional player usage</caption>
        ...
        <nav>
          <ul>
            ...
            {shapeMice.slice(0, 5).map(m => <li key={m.id}><a href={`/keyboards/${sl(m.name)}`}>{m.name} Review</a></li>)}
          </ul>
        </nav>
      </article>
